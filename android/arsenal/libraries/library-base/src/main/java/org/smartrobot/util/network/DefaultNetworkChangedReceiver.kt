package org.smartrobot.util.network
...
class DefaultNetworkChangedReceiver : BroadcastReceiver() {
    ...
    var mLastNetworkType: DefaultNetworkUtil.MNetworkType? = null

    companion object {

        @Throws(Exception::class)
        fun register(context: Context): DefaultNetworkChangedReceiver {
            val filter = IntentFilter()
            filter.addAction(WifiManager.NETWORK_STATE_CHANGED_ACTION)
            filter.addAction(WifiManager.WIFI_STATE_CHANGED_ACTION)
            filter.addAction(ConnectivityManager.CONNECTIVITY_ACTION)
            val networkChangedReceiver = DefaultNetworkChangedReceiver()
