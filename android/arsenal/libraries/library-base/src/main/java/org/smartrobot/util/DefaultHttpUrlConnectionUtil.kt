...
@Suppress("unused")
object DefaultHttpUrlConnectionUtil {
    var TAG = "DefaultHttpUrlConnectionUtil"

    init {
        DefaultLogUtil.setDebugAble(true)
    }
    ...
    fun post(httpUrl: String, params: Map<String, String>, files: Map<String, File>?): String {
        ...
        DefaultLogUtil.d(TAG, "responseCode:" + responseCode)
        DefaultLogUtil.d(TAG, "responseMessage:" + conn.responseMessage)
        outStream.close()
        conn.disconnect()
    ...
    operator fun get(httpUrl: String): String {
        ...
        try {
            ...
            DefaultLogUtil.d(TAG, "responseContent:" + content.toByteArray(charset("utf-8")))
        } catch (e: Exception) {
            e.printStackTrace()
        }
