...
export default class HelloWorld extends React.Component {
    ...
    render() {
        return (
            <Navigator
                initialRoute={{ title: 'RN PAGE NO.', index: 0 }}
                renderScene={(route, navigator) =>
                    <TitleBar
                        title={'RN PAGE NO.'+route.index} // Function to call when a new scene should be displayed

                        onForward={ () => {
                          const nextIndex = route.index + 1;
                          navigator.push({
                            title: 'RN PAGE NO.' + nextIndex,
                            index: nextIndex,
                          });
                        }}
